...
import { Layout, Box, Image as ImageIcon, Wand2, Layers, Plus, Trash2, Download, History, Sparkles, Shirt, Move, Maximize, RotateCcw, Zap, Cpu, ArrowRight, ArrowUp, ArrowDown, Globe, Scan, Camera, Aperture, Repeat, SprayCan, Triangle, Package, Menu, X, Check, MousePointer2, Undo2, Redo2, Save } from 'lucide-react';
...
export default function App() {
  ...
  const [assets, setAssets] = useState<Asset[]>(() => loadState('sku_assets', []));
  const [generatedMockups, setGeneratedMockups] = useState<GeneratedMockup[]>(() => loadState('sku_generatedMockups', []));
  
  const [selectedProductId, setSelectedProductId] = useState<string | null>(() => loadState('sku_selectedProductId', null));
  const [placedLogos, setPlacedLogos] = useState<PlacedLayer[]>(() => loadState('sku_placedLogos', []));
  const [prompt, setPrompt] = useState(() => loadState('sku_prompt', ''));
  
  // Skip intro if we have saved data
  const [showIntro, setShowIntro] = useState(() => {
    // Check if we have any substantial saved data
    const hasData = localStorage.getItem('sku_assets') || localStorage.getItem('sku_placedLogos');
    return !hasData;
  });
  ...
  // Ref to access latest state in event listeners
  const placedLogosRef = useRef<PlacedLayer[]>(placedLogos); 
  ...
  const [future, setFuture] = useState<PlacedLayer[][]>([]);
  ...
  const dragStartSnapshot = useRef<PlacedLayer[] | null>(null); // To store state before drag starts

  // State for Wheel/Scaling Undo
  const wheelStartSnapshot = useRef<PlacedLayer[] | null>(null);
  const wheelTimeout = useRef<NodeJS.Timeout | null>(null);

  // Demo intro sequence
  useEffect(() => {
    if (!showIntro) return; // Skip logic if intro is disabled
  ...
  const saveProject = () => {
    try {
      localStorage.setItem('sku_assets', JSON.stringify(assets));
      localStorage.setItem('sku_placedLogos', JSON.stringify(placedLogos));
      localStorage.setItem('sku_selectedProductId', JSON.stringify(selectedProductId));
      localStorage.setItem('sku_prompt', prompt);
      localStorage.setItem('sku_generatedMockups', JSON.stringify(generatedMockups));
      setNotification({ message: 'Project saved successfully', type: 'success' });
    } catch (e) {
      console.error(e);
      if (e instanceof DOMException && e.name === 'QuotaExceededError') {
        setNotification({ message: 'Storage full. Try removing some assets.', type: 'error' });
      } else {
  ...
  const pushToHistory = (newState: PlacedLayer[]) => {
  ...
  const handleUndo = () => {
    ...
    setFuture(prev => [placedLogos, ...prev]);
    setPlacedLogos(previous);
  ...
  const handleRedo = () => {
    ...
    setHistory(prev => [...prev, placedLogos]);
    setPlacedLogos(next);
    setFuture(newFuture);
  };
  ...
  const moveLayer = (uid: string, direction: 'up' | 'down', e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    
    const index = placedLogos.findIndex(l => l.uid === uid);
    if (index === -1) return;
    if (direction === 'up' && index === placedLogos.length - 1) return; // Topmost
    if (direction === 'down' && index === 0) return; // Bottommost

    pushToHistory(placedLogos); // Save current state
  ...
  const handleStart = (clientX: number, clientY: number, layer: PlacedLayer) => {
    // Snapshot state before drag starts
    dragStartSnapshot.current = placedLogos;
  ...
  const handleWheel = (e: React.WheelEvent, layerId: string) => {
     ...
     // Snapshot state before a sequence of wheel events
     if (!wheelTimeout.current) {
        wheelStartSnapshot.current = placedLogos;
     }
     ...
     // Set timeout to commit history after scrolling stops
     wheelTimeout.current = setTimeout(() => {
        if (wheelStartSnapshot.current) {
            // We push the *original* state (before scroll) to history
            // The current state (after scroll) remains as 'present'
            const snapshot = wheelStartSnapshot.current;
            setHistory(prev => [...prev, snapshot]);
            setFuture([]);
            wheelStartSnapshot.current = null;
        }
  ...
  useEffect(() => {
    ...
    const onMouseUp = () => {
      // Logic to commit history on drag end
      
      if (dragStartSnapshot.current) {
          const currentStr = JSON.stringify(placedLogosRef.current);
          const startStr = JSON.stringify(dragStartSnapshot.current);
    ...
    const onTouchEnd = () => {
      // Same history logic as onMouseUp
      if (dragStartSnapshot.current) {
        const currentStr = JSON.stringify(placedLogosRef.current);
        const startStr = JSON.stringify(dragStartSnapshot.current);
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans flex overflow-hidden relative">
      ...
      {/* Toast Notification */}
      {notification && (
        <div className={`fixed top-20 left-1/2 -translate-x-1/2 z-[60] px-6 py-3 rounded-full shadow-2xl flex items-center gap-2 animate-fade-in
                ...
                <div className="w-full lg:w-80 flex flex-col gap-6 glass-panel p-6 rounded-2xl overflow-y-auto flex-1 lg:flex-none">
                   <div>
                      <div className="flex items-center justify-between mb-4">
                         <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider">1. Select Product</h3>
                         {/* Save Button */}
                         <button 
                           onClick={saveProject}
                           className="flex items-center gap-1 text-xs font-medium text-indigo-400 hover:text-indigo-300 px-2 py-1 rounded bg-indigo-500/10 hover:bg-indigo-500/20 transition-colors"
                ...
                <div className="h-[45vh] lg:h-auto lg:flex-1 glass-panel rounded-2xl flex items-center justify-center bg-zinc-900 relative overflow-hidden select-none flex-shrink-0">
                   ...
                   <div className="absolute top-4 right-4 z-30 flex gap-2">
                      <button
                         ...
                         className={`p-2 rounded-lg bg-zinc-800 border border-zinc-700 transition-colors ${history.length === 0 ? 'opacity-50 cursor-not-allowed text-zinc-500' : 'hover:bg-zinc-700 text-white shadow-lg'}`}
                         title="Undo"
                      >
                         <Undo2 size={18} />
                      </button>
                      <button
                         ...
                         className={`p-2 rounded-lg bg-zinc-800 border border-zinc-700 transition-colors ${future.length === 0 ? 'opacity-50 cursor-not-allowed text-zinc-500' : 'hover:bg-zinc-700 text-white shadow-lg'}`}
                         title="Redo"
                      >
                         <Redo2 size={18} />
                      </button>
                            ...
                            return (
                               ...
                               >
                                  ...
                                  {placedLogos.length > 1 && (
                                     <div 
                                        className="absolute -top-10 left-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-50"
                                        onMouseDown={e => e.stopPropagation()}
                                        onTouchStart={e => e.stopPropagation()}
                                     >
                                        <button
                                            onClick={(e) => moveLayer(layer.uid, 'down', e)}
                                            disabled={index === 0}
                                            className={`p-1.5 rounded bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-700 shadow-sm ${index === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
                                            title="Send Backward"
                                        >
                                            <ArrowDown size={14} />
                                        </button>
                                        <button
                                            onClick={(e) => moveLayer(layer.uid, 'up', e)}
                                            disabled={index === placedLogos.length - 1}
                                            className={`p-1.5 rounded bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-700 shadow-sm ${index === placedLogos.length - 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
                                            title="Bring Forward"
                                        >
                                            <ArrowUp size={14} />
                                        </button>
